...
class Dictamen_trazos_usos extends Controller
{

    public static function solicitud()
    {

        if ($folio = Dictamen_trazos_usos_model::solicitud()) {
            ...
            if ($result) {
                ...
                $result2 = Solicitudes_model::consulta_datos_solicitud($folio, 11, $id_etapa);
    ...
    public function ingresa_solicitud(Request $request)
    {
        ...
        if ($response = Dictamen_trazos_usos_model::ingresa_solicitud($request)) {
            $obj = $response[0];
            ...
            if ($obj->idcaptura > 0) {
                ...
                $rows = Solicitudes_model::actualiza_datos_solicitud($request, 11, $request->id_solicitud, $request->etapa);
                ...
                if ($rows == 0) {
                    ...
                    Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 61, 'pendiente', $obj->idcaptura, null);
    ...
    public function ingresa_tramite(Request $request)
    {
        ...
        $requisitos = Dictamen_trazos_usos_model::consulta_requisitos_dtu($request->id_solicitud);
        ...
        foreach ($_FILES as $key => $file) {
            ...
            if ($file["size"] > 0 && $file["name"] != "") {
                ...
                $rows += Dictamen_trazos_usos_model::inserta_requisito_ord(
                    $id_documento,
                    $filename,
                    $ext,
        ...
        $pendientes = Dictamen_trazos_usos_model::consulta_archivos_faltantes($request->id_solicitud);

        if ($pendientes || $rows <> $files_s3) {

            Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 62, 'pendiente', $id_captura, null);
            ...
            if ($result) {
                ...
                $result2 = Solicitudes_model::consulta_datos_solicitud($request->id_solicitud, $id_tramite, 60);

                $vars = [
                    'files'    => Dictamen_trazos_usos_model::get_files($id_tramite, $folio),
                    'folio'    => $folio,
            ...
            Dictamen_trazos_usos_model::actualiza_edo_act($request->id_captura, $ing);
            Dictamen_trazos_usos_model::notifica($request, $titulo, $mensaje, $correo);
    ...
    {
        if ($response = Dictamen_trazos_usos_model::actualiza_solicitud_dtu($request)) {
            ...
            if ($obj->idcaptura > 0) {
                ...
                $rows = Solicitudes_model::actualiza_datos_solicitud($request, 11, $request->id_solicitud, 61);
    ...
    public function upload(Request $request)
    {
        ...
        if ($response = Dictamen_trazos_usos_model::upload($request->all())) {
            http_response_code(200);
