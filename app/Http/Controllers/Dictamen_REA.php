...
use App\model\Dictamen_rea as ModelDictamen_rea;
use App\model\Dictamen_trazos_usos_model;
use PDF;
use App\model\Dictamen_rea_Model;

class Dictamen_rea extends Controller
{
    function Conectarse()
    {
    ...
    {
        if ($folio = Dictamen_rea_Model::solicitud()) {
    ...
    public function carta($fecha, $id_captura)
    {
        ...
        $pdf = PDF::loadView('dictamen_rea.carta', $data);
    ...
    {
        if ($response = Dictamen_rea_Model::ingresa_solicitud($request)) {
            $obj = $response[0];
            ...
            if ($obj->idcaptura > 0) {
                ...
                if ($rows == 0) {
                    ...
                    Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 109, 'pendiente', $obj->idcaptura, null);
    ...
    public function actualiza_solicitud(Request $request)
    {
        ...
        if ($response = Dictamen_rea_Model::actualiza_solicitud($request)) {

            $obj = $response[0];

            if ($obj->idcaptura > 0) {
                $rows = Solicitudes_model::actualiza_datos_solicitud($request, 1, $request->id_solicitud, 107);
                ...
                if ($rows == 0) {
                    ...
                    Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 107, 'pendiente', $obj->idcaptura, null);
                    http_response_code(200);
                    echo json_encode($obj->idcaptura);
                }
            } else {
    ...
    public function ingresa_tramite(Request $request)
    {
        ...
        $requisitos = Dictamen_rea_Model::consulta_requisitos_op(
        ...
        foreach ($requisitos as $r) {
            ...
            if ($r->estatus != 'validado') {
                ...
                if (Storage::disk('s3')->exists(env('AWS_FOLDER') . session('id_usuario') . '/' . $nombre_archivo)) {
                    ...
                    Storage::disk('s3')->delete(env('AWS_FOLDER') . session('id_usuario') . '/' . $nombre_archivo);
        ...
        foreach ($_FILES as $key => $file) {
            ...
            if ($file["size"] > 0 && $file["name"] != "") {
                ...
                $rows += Dictamen_rea_Model::inserta_requisito_op(
                    $id_documento,
                    $filename,
                    $ext,
                    $request->id_solicitud
        ...
        $pendientes = Dictamen_rea_Model::consulta_archivos_faltantes($request->id_solicitud);

        if ($pendientes || $rows <> $files_s3) {

            // Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 3, 'pendiente');
            Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 108, 'pendiente', $id_captura, null);
            ...
            if ($result) {
                ...
                $result2 = Solicitudes_model::consulta_datos_solicitud($request->id_solicitud, $id_tramite, 106);

                $vars = [
                    'files'    => Dictamen_rea_Model::get_files($id_tramite, $folio),
                    'folio'    => $folio,
            ...
            Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 109, 'en revision', $id_captura, 1);

            if ($request->id_etapa == 111) {
            ...
            Dictamen_rea_Model::actualiza_edo_act($request->id_captura, $ing);
            Dictamen_rea_Model::notifica($request, $titulo, $mensaje, $correo);
