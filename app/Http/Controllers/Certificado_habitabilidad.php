...
use App\model\Certificado_habitabilidad_model;
...
class Certificado_habitabilidad extends Controller
{
    public $_ID_TRAMITE=13;
    public function solicitud()
    {

        if ($folio = Certificado_habitabilidad_model::solicitud()) {

            $vars = [
                'predios'  => Predios_model::get_all(0),
                'ultimo'   => Predios_model::get_count(),
                'files'    => Certificado_habitabilidad_model::get_files($folio),
                'folio'    => $folio
            ];            
            //echo"<br>$folio<br>";
    ...
    public function upload(Request $request)
    {
        ...
        if ($response = Certificado_habitabilidad_model::upload($request->all())) {
    ...
    public function ingresa_solitud(Request $request)
    {
        ...
        if ($response = Certificado_habitabilidad_model::ingresa_solitud_op($request)) {
            $obj = $response[0];
            ...
            if ($obj->idcaptura > 0) {
                ...
                if ($rows == 0) {
                    ...
                    Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 74, 'pendiente', $obj->idcaptura, null);
                    http_response_code(200);
    ...
    public function ingresa_tramite(Request $request)
    {
        ...
        $requisitos = Certificado_habitabilidad_model::consulta_requisitos_op(
            $request->id_captura
        );
        ...
        $rows_elimina = Certificado_habitabilidad_model::elimina_requisito_op(
            $request->id_captura
        );
        ...
        foreach ($_FILES as $key => $file) {
            ...
            if ($file["size"] > 0 && $file["name"] != "") {
                ...
                if (move_uploaded_file($_FILES[$key]["tmp_name"], "\\\\172.16.4.125\\RespaldosBD\\ZAPOPC\\licenciaobp\\ch\\" . $filename)) {
                    $val= Certificado_habitabilidad_model::inserta_requisito_op(
                        $request->id_captura,
                        $id_documento,
                        $ruta_completa,
                        $filename,
        ...
        $pendientes = Certificado_habitabilidad_model::consulta_archivos_faltantes($request->id_solicitud);
...
//        var_dump($pendientes);
        ...
        $rows <> $files_s3
            ...
            Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 75, 'pendiente', $id_captura, null);
            ...
            Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 76, 'en revision', $id_captura, 1);

            if ($request->id_etapa == 78) {
            ...
            $mensaje = '<font color="#000000">Gracias  por utilizar esta herramienta electrónica. Has </font><font color="#000000">' .
             ...
             Certificado de Habitibilidad</strong>.</font><br><br><font color="#000000"> 
            ...
            Certificado_habitabilidad_model::actualiza_edo_act($request->id_captura, $ing);
            Certificado_habitabilidad_model::notifica($request, $titulo, $mensaje, $correo);
    ...
    public function actualiza_solitud(Request $request)
    {
        ...
        if ($response = Certificado_habitabilidad_model::actualiza_solitud_op($request)) {
            ...
            if ($obj->idcaptura > 0) {
                ...
                $rows = Solicitudes_model::actualiza_datos_solicitud($request, $this->_ID_TRAMITE , $request->id_solicitud, 74);
    ...
    public function carta($fecha, $id_captura)
    {
        ...
        $pdf = PDF::loadView('certificado_habitabilidad.carta', $data);
        return $pdf->download('carta responsiva.pdf');
    }
    public function get_by_licencia(Request $request){
        $result = Certificado_habitabilidad_model::get_by_licencia(strval($request->clave));
