...
class trabajos_menores extends Controller
{
    ...
    public function solicitud()
    {
        ...
        if ($folio = Trabajos_menores_model::solicitud()) {
    ...
    public function upload(Request $request)
    {
        ...
        if ($response = Trabajos_menores_model::upload($request->all())) {
    ...
    public function ingresa_solitud(Request $request)
    {
        ...
        if ($response = Trabajos_menores_model::ingresa_solitud_op($request)) {
            $obj = $response[0];
            ...
            if ($obj->idcaptura > 0) {
                ...
                if ($rows == 0) {
                    ...
                    Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 2, 'pendiente', $obj->idcaptura, null);
                    http_response_code(200);
    ...
    public function ingresa_tramite(Request $request)
    {
        ...
        $requisitos = Trabajos_menores_model::consulta_requisitos_op(
            $request->id_captura
        );
        ...
        $rows_elimina = Trabajos_menores_model::elimina_requisito_op(
            $request->id_captura
        );
        ...
        foreach ($_FILES as $key => $file) {
            ...
            if ($file["size"] > 0 && $file["name"] != "") {
                ...
                if (move_uploaded_file($_FILES[$key]["tmp_name"], "\\\\172.16.4.125\\RespaldosBD\\ZAPOPC\\licenciaobp\\tw\\" . $filename)) {
                    $rows += Trabajos_menores_model::inserta_requisito_op(
                        $request->id_captura,
                        $id_documento,
                        $ruta_completa,
                        $filename,
        ...
        $pendientes = Trabajos_menores_model::consulta_archivos_faltantes($request->id_solicitud);

        if ($pendientes || $rows <> $files_s3) {

            // Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 3, 'pendiente');
            Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 3, 'pendiente', $id_captura, null);
            ...
            if ($result) {
                ...
                if ($id_tramite == 1) {
                    ...
                    $vars = [
                        ...
                        'files'    => Trabajos_menores_model::get_files($id_tramite, $folio),
                        'folio'    => $folio,
            ...
            Solicitudes_model::actualiza_etapa_solicitud($request->id_solicitud, 4, 'en revision', $id_captura, 1);

            if ($request->id_etapa == 6) {
    ...
    public function actualiza_solitud(Request $request)
    {
        ...
        if ($response = Trabajos_menores_model::actualiza_solitud_op($request)) {
            ...
            if ($obj->idcaptura > 0) {
                ...
                $rows = Solicitudes_model::actualiza_datos_solicitud($request, 1, $request->id_solicitud, 2);
    ...
    public function carta($fecha, $id_captura)
    {
        ...
        $pdf = PDF::loadView('trabajos_menores.carta', $data);
