...
class Certificado_habitabilidad_model extends Model
{
    ...
    public static function get_files($id_solicitud)
    {
        ...
        $pendientes = DB::select('SELECT c.id_cat_archivo, c.nombre, c.id_tramite, c.descripcion_larga, 
            ...
            ) and ((c.id_documento>0 and c.id_tramite = 13) or (c.id_documento>0 and c.id_tramite = 0))');
    ...
    public static function solicitud()
    {
        ...
        if (session('lastpage') !== null && session('lastpage') == __FILE__) {
            ...
            $result = Solicitudes_model::consulta_ultimo_folio(13);
            $folio = $result[0]->folio;
        } else {
            $id_revisor = Solicitudes_model::balanza(73);
    ...
    {
        $sql = "EXECUTE capturaweb.dbo.ch_inserta_certhab ?,?,?,?
                ,?,?,?,?
                ,?,?,?,?
                ,?,?,?,?
                ,?,?,?,?
    ...
    public static function actualiza_solitud_op($request)
    {
        ...
        $sql = "EXECUTE  capturaweb.dbo.ch_actualiza_certhab ?,?,?,?
        ,?,?,?,?
        ,?,?,?,?
        ,?,?,?,?
        ,?,?,?,?
    ...
    {
        $sql = "execute capturaweb.dbo.ch_elimina_requisitos_pendientes_vdgital ? ";
    ...
    public static function inserta_requisito_op($id_precaptura, $id_documento, $ruta, $filename, $extension, $id_solicitud)
    {
        ...
        ((c.id_documento=' . $id_documento . ' and c.id_tramite = 13) 
        ...
        $pend_file = DB::select('SELECT c.id_cat_archivo, c.nombre, c.id_tramite, c.descripcion_larga, c.id_documento FROM cat_archivo as c WHERE NOT EXISTS (SELECT 1 FROM archivos as a 
                ...
                and a.id_solicitud =' . $id_solicitud . ' 
                and a.id_usuario =' . session('id_usuario') . '
                ) and ((c.id_documento='.$id_documento.' and c.id_tramite = 13) or (c.id_documento='.$id_documento.' and c.id_tramite = 0))');
        ...
        if ($result1 && $result1[0]->num > 0) {
            ...
            //$res1 = DB::select('SELECT c.id_cat_archivo FROM cat_archivo as c WHERE c.id_tramite = 13 and c.id_documento = ' . $id_documento);
        } else {
        ...
        if ($res1&&$result) {
            ...
            and a.id_solicitud =' . $id_solicitud . ' 
            and a.id_usuario =' . session('id_usuario') . '
            ) and c.id_documento=' . $id_documento . ' and c.id_tramite=13');
            */
    ...
    public static function consulta_archivos_faltantes($id_solicitud)
    {
        ...
        and a.id_solicitud =' . $id_solicitud . '         
        and a.id_usuario =' . session('id_usuario') . '
        ) and c.id_documento>0 and c.obligatorio=1 and c.id_tramite = 13');
        */
        ...
        $pendientes = DB::select('SELECT c.id_cat_archivo, c.nombre, c.id_tramite, c.descripcion_larga, 
            ...
            and a.id_solicitud =' . $id_solicitud . '         
            and a.id_usuario =' . session('id_usuario') . '
            ) and ((c.id_documento>0 and c.id_tramite = 13 and c.obligatorio=1) or (c.id_documento>0 and c.id_tramite = 0))');
    ...
    public static function consulta_requisitos_validados($id_solicitud)
    {
        ...
        $result = DB::connection('tramites_op')->select($sql, [$id_solicitud, 5]);
        return $result;
    }
    public static function consulta_requisitos_op($id_captura)
    {
        ...
        $result = DB::connection('tramites_op')->select($sql, [$id_captura, 5]);
        return $result;
    }
    public static function notifica($request, $titulo, $mensaje, $correo)
    {
        ...
        if (DB::table('notificaciones')->insert($data)) {
            ...
            return DB::table('solicitudes')
                ...
                ->update(['id_etapa' => 76, 'estatus' => 'en revision']);
        } else {
            return false;
        }
    }
