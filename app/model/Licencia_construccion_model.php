...
class Licencia_construccion_model extends Model
{
    ...
    public static function get_files($id_solicitud)
    {
        ...
        $pendientes = DB::select('SELECT c.id_cat_archivo, c.nombre, c.id_tramite, c.descripcion_larga, 
            ...
            ) and c.id_documento > 0 and c.id_tramite = 2 ');

        $resv = Licencia_construccion_model::consulta_requisitos_validados($id_solicitud);
    ...
    public static function solicitud()
    {
        ...
        if (session('lastpage') !== null && session('lastpage') == __FILE__) {
            ...
            $result = Solicitudes_model::consulta_ultimo_folio(2);
            $folio = $result[0]->folio;
        } else {

            $id_revisor = Solicitudes_model::balanza(22);
    ...
    {
        $sql = "EXECUTE tw_inserta_licencia_construccion ?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?,?,?,?,?,?";
        ...
        $params = [
            ...
            $tipo_persona,
            $request->txtcpropiedadNumero,
            $request->txtcpropiedadNotario,
            $request->txtcpropiedadMunicipio,
            $request->txtcpropiedadTransmision,
            $request->directorNombre,
            $request->directordomicilio,
            $request->directortelefono,
            $request->directorRegistro,
            $request->txtsiapa,
            $request->regimen
        ];

        return DB::connection('tramites_op')->select($sql, $params);
    }

    public static function actualiza_solitud_op($request)
    {

        $sql = "EXECUTE tw_actualiza_licencia_construccion ?,?,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?";
    ...
    public static function actualiza_edo_act($id_captura, $ing)
    {
        ...
        $params = [$id_captura];
        return DB::connection('tramites_op')->update($sql, $params);
    }
    ...
    public static function elimina_requisito_op($id_precaptura)
    {
        ...
        $params = [$id_precaptura, 2];
        return DB::connection('tramites_op')->delete($sql, $params);
    }
    ...
    public static function inserta_requisito_op($id_precaptura, $id_documento, $ruta, $filename, $extension, $id_solicitud)
    {
        ...
        $params1 = [$id_precaptura, $id_documento];
        ...
        if ($result1 && $result1[0]->num > 0) {
            ...
            $params = [$ruta, $id_precaptura, $id_documento];
            $result = DB::connection('tramites_op')->update($sql, $params);
            $res1   = DB::select('SELECT c.id_cat_archivo FROM cat_archivo as c WHERE c.id_tramite = 2 and c.id_documento = ' . $id_documento);
            ...
            if ($res1) {
                ...
                and a.id_solicitud =' . $id_solicitud . ' 
                and a.id_usuario =' . session('id_usuario') . '
                ) and c.id_tramite = 2 and c.id_documento=' . $id_documento);
            ...
            $params = [$id_precaptura, $id_documento, $ruta];
    ...
    public static function consulta_archivos_faltantes($id_solicitud)
    {
        ...
        and a.id_solicitud =' . $id_solicitud . '         
        and a.id_usuario =' . session('id_usuario') . '
        ) and c.id_documento> 0 and c.obligatorio=1 and c.id_tramite = 2');
    }
    ...
    public static function consulta_requisitos_validados($id_solicitud)
    {
        ...
        return DB::connection('tramites_op')->select($sql, [$id_solicitud, 2]);
    }

    public static function consulta_requisitos_op($id_captura)
    {
        $sql = "execute CAPTURAWEB.dbo.tw_consulta_requisitos ?,?";
        return DB::connection('tramites_op')->select($sql, [$id_captura, 2]);
    }
    ...
    public static function notifica($request, $titulo, $mensaje, $correo)
    {
        ...
        if (DB::table('notificaciones')->insert($data)) {
            ...
            return DB::table('solicitudes')
                ...
                ->update(['id_etapa' => 22, 'estatus' => 'en revision']);
        } else {
            return false;
        }
    }
