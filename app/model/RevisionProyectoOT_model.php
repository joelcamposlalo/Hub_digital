...
class RevisionProyectoOT_model extends Model
{
    /**
    ...
    public static function get_files($id_solicitud)
    {
      $terminados = DB::table('archivos as a')
            ...
            ->where([
                ...
                ['c.id_tramite', '=', RevisionProyectoOT_model::_ID_TRAMITE],
        ...
        $pendientes = DB::select('SELECT c.id_cat_archivo, c.nombre, c.id_tramite, c.descripcion_larga, 
            ...
            ) and c.id_documento>0  and c.id_tramite='.RevisionProyectoOT_model::_ID_TRAMITE);
        
        $resv= RevisionProyectoOT_model::consulta_requisitos_validados($id_solicitud);
        $validados=array();
    ...
    public static function solicitud()
    {
        ...
        if (session('lastpage') !== null && session('lastpage') == __FILE__) {
            ...
            $result = Solicitudes_model::consulta_ultimo_folio(15);
            $folio = $result[0]->folio;
        } else {
            $id_revisor = Solicitudes_model::balanza(93);
            $folio = DB::table('solicitudes')->insertGetId([
                'id_tramite' => RevisionProyectoOT_model::_ID_TRAMITE,
                'id_usuario' => session('id_usuario'),
                'id_etapa'   =>  93,
                'estatus'    =>  'pendiente',
    ...
    {
        $sql = "EXECUTE capturaweb.dbo.tw_inserta_revision_proyecto_ot ?,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?";
        ...
        $params = array(
            ...
            $request->calle_1, $request->calle_2, $request->dictamen, $request->alineamiento, $request->cuenta, $request->curt,
            $request->giro   ,$request->casas_habitacion ,  $request->nombre, $request->apellido_p, $request->apellido_m, $request->domicilio, $request->telefono, $request->correo,
            session('id_usuario'), $request->nombre, $request->apellido_p, $request->apellido_m, $request->id_solicitud, $tipo_persona
        );
    ...
    public static function actualiza_solitud_op($request)
    {
        ...
        $sql = "EXECUTE capturaweb.dbo.tw_actualiza_revision_proyecto_ot ?,?,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?
                ,?,?,?,?,?,?";
        ...
        $params = array(
            ...
            //$request->privativa, 
            $request->calle_1, $request->calle_2, $request->dictamen, $request->alineamiento, $request->cuenta, $request->curt,
            $request->giro, $request->nombre, $request->apellido_p, $request->apellido_m, $request->domicilio, $request->telefono, $request->correo,
            session('id_usuario'), $request->nombre, $request->apellido_p, $request->apellido_m, $request->id_solicitud, $request->id_captura,$request->casas_habitacion 
        );
    ...
    public static function inserta_requisito_op($id_precaptura, $id_documento, $ruta, $filename, $extension, $id_solicitud)
    {
        ...
        if ($result1 && $result1[0]->num > 0) {
            ...
            $res1 = DB::select('SELECT c.id_cat_archivo FROM cat_archivo as c WHERE c.id_tramite='.RevisionProyectoOT_model::_ID_TRAMITE.' and c.id_documento = ' . $id_documento);
            if ($res1) {
                ...
                and a.id_solicitud =' . $id_solicitud . ' 
                and a.id_usuario =' . session('id_usuario') . '
                ) and c.id_tramite=15 and c.id_documento=' . $id_documento);
            ...
            $res1 = DB::select('SELECT c.id_cat_archivo FROM cat_archivo as c WHERE  c.id_tramite=15 and  c.id_documento = ' . $id_documento);
            if ($res1) {
                $id_cat_archivo = $res1[0]->id_cat_archivo;
                ...
                and a.id_solicitud =' . $id_solicitud . ' 
                and a.id_usuario =' . session('id_usuario') . '
                ) and c.id_tramite='.RevisionProyectoOT_model::_ID_TRAMITE.'  and c.id_documento=' . $id_documento);
    ...
    public static function consulta_archivos_faltantes($id_solicitud)
    {
        ...
        and a.id_solicitud =' . $id_solicitud . '         
        and a.id_usuario =' . session('id_usuario') . '
        ) and c.id_documento>0 and c.obligatorio=1 and c.id_Tramite='.RevisionProyectoOT_model::_ID_TRAMITE);
    ...
    public static function consulta_requisitos_validados($id_solicitud)
    {
        ...
        $result = DB::connection('tramites_op')->select($sql, [$id_solicitud, 11]);
        return $result;
    }
    public static function consulta_requisitos_op($id_captura)
    {
        $sql="execute CAPTURAWEB.dbo.tw_consulta_requisitos ?,?";
        $result = DB::connection('tramites_op')->select($sql, [$id_captura, 11]);
        return $result;
    }
    public static function notifica($request, $titulo, $mensaje, $correo)
    {
        ...
        if (DB::table('notificaciones')->insert($data)) {
            ...
            return DB::table('solicitudes')
                ...
                ->update(['id_etapa' => 36, 'estatus' => 'en revision']);
        } else {
            return false;
        }
    }
