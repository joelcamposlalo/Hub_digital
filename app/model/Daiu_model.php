...
class Daiu_model extends Model
{
    public static function solicitud()
    {
        //Verifica si la pagina fue refrescada
        ...
        //Si la pagina fue refrescada entonces se obtiene el folio de la solicitud
        if (session('lastpage') !== null && session('lastpage') == __FILE__) {
            $result = Solicitudes_model::consulta_ultimo_folio(38);
    ...
    public static function get_files($id_solicitud)
    {
        ...
        $pendientes = DB::select('SELECT c.id_cat_archivo, c.nombre, c.id_tramite, c.descripcion_larga,
            ...
            ) and c.id_documento>0 and c.id_tramite = 38 and c.universal=false and c.id_tramite = 38');
    ...
    public static function actualizarEtapaSolicitud($id_solicitud, $nueva_etapa)
    {
        ...
        DB::table('solicitudes')
            ...
            ->update([
                'id_etapa' => $nueva_etapa
            ]);

        // Agrega al historial
        DB::table('solicitudes_hist')->insert([
    ...
    // NUEVO
    public static function consultaCuentaCatastro($cuenta)
    {

        $sql = "SELECT * FROM vd_consulta_cuenta(?, ?);";

        if (strlen($cuenta) == 10) {
            return DB::connection('catastro')->select($sql, [$cuenta, null]);
        } elseif (strlen($cuenta) == 31) {
            return DB::connection('catastro')->select($sql, [null, $cuenta]);
        }
