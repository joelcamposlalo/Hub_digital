...
class Dictamen_rea_Model extends Model
{
    public static function solicitud()
    {
        ...
        if (session('lastpage') !== null && session('lastpage') == __FILE__) {
            $result = Solicitudes_model::consulta_ultimo_folio(17);
            $folio = $result[0]->folio;
        } else {
           // $id_revisor = Solicitudes_model::balanza(106);
    ...
    public static function get_files($id_solicitud)
    {
        ...
        $pendientes = DB::select('SELECT c.id_cat_archivo, c.nombre, c.id_tramite, c.descripcion_larga, 
            ...
            ) and c.id_documento>0 and c.id_tramite = 17');
        ...
        join archivosma ao  on ao.id_archivo =a.id_archivo 
    ...
    public static function consulta_requisitos_op($id_solicitud)
    {
        ...
        on c.id_cat_archivo =a.id_cat_archivo join archivosma ao  on ao.id_archivo =a.id_archivo 
    ...
    {
        $sql = "EXECUTE rea_inserta_dictrea ?,?,?,?,?,
        ?,?,?,?,?,
        ?,?,?,?,?,
        ?,?,?,?,?
        ,?,?";
    ...
    public static function actualiza_solicitud($request)
    {
        ...
        $sql = "EXECUTE rea_actualiza_dictrea ?,?,?,?,?,
        ?,?,?,?,?,
        ?,?,?,?,?,
        ?,?,?,?,?";
        ...
        $params = array(
            ...
            $request->nombre, $request->apellido_p, $request->apellido_m, $request->id_captura, $request->correo
        );
    ...
    public static function inserta_requisito_op($id_documento, $filename, $extension, $id_solicitud)
    {
        ...
        c.id_documento = ' . $id_documento . ' and id_tramite=17');
        if ($res1) {
            ...
            and a.id_solicitud =' . $id_solicitud . ' 
            and a.id_usuario =' . session('id_usuario') . '
            ) and c.id_tramite=17 and c.id_documento=' . $id_documento);
    ...
    public static function consulta_archivos_faltantes($id_solicitud)
    {
        ...
        and a.id_solicitud =' . $id_solicitud . '         
        and a.id_usuario =' . session('id_usuario') . '
        ) and c.id_documento>0 and c.obligatorio=1 and c.id_tramite = 17');
        return $pendientes;
    }
    ...
    public static function notifica($request, $titulo, $mensaje, $correo)
    {
        ...
        if (DB::table('notificaciones')->insert($data)) {
            ...
            return DB::table('solicitudes')
                ...
                ->update(['id_etapa' => 109, 'estatus' => 'en revision']);
        } else {
            return false;
        }
    }
