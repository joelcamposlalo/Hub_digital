...
class Dictamen_img_urbana_model extends Model
{
    use HasFactory;
    ...
    public static function solicitud()
    {
        ...
        if (session('lastpage') !== null && session('lastpage') == __FILE__) {
            ...
            $result = Solicitudes_model::consulta_ultimo_folio(10);
            $folio = $result[0]->folio;
        } else {
            $id_revisor = Solicitudes_model::balanza(55);
    ...
    public static function get_files($id_solicitud)
    {
        ...
        ) and ((c.id_documento>0 and c.id_tramite = 10))
        ...
        //$resv = Dictamen_img_urbana_model::consulta_requisitos_validados($id_solicitud);
        $validados = array();
    ...
    /**
     * Ingresa la solicitud a Ordenamiento
     */

    public static function ingresa_solitud_ord($request)
    {
        $sql = "EXECUTE diu_inserta_dictimagen  ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?";
        ...
        $params = array(
            $request->calle, $numero, $request->fraccionamiento, $request->calle_1, $request->calle_2,
            $request->cuenta, $request->curt, $request->domicilio, $request->telefono, $request->correo,
            session('id_usuario'), $request->nombre, $request->apellido_1, $request->apellido_2, $tipo_persona,
    ...
    public static function actualiza_solitud_ord($request)
    {

        $sql = "EXECUTE diu_actualiza_dictimagen ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?";
        ...
        $params = array(
            $request->calle, $numero, $request->fraccionamiento, $request->calle_1, $request->calle_2,
            $request->cuenta, $request->curt, $request->domicilio, $request->telefono, $request->correo,
            session('id_usuario'), $request->nombre, $request->apellido_1, $request->apellido_2,
            $request->id_solicitud, $request->id_captura
        );
    ...
    public static function elimina_requisito_ord($id_precaptura)
    {
        $sql = "execute diu_elimina_requisitos_pendientes_vdgital ? ";
        $params = array($id_precaptura);
        $result = DB::connection('captura_op')->delete($sql, $params);
        return $result;
    }
    ...
    public static function inserta_requisito_ord($id_documento, $filename, $extension, $id_solicitud)
    {
        ...
        c.id_documento = ' . $id_documento . ' and id_tramite=10');
        ...
        if ($res1) {
            ...
            and a.id_solicitud =' . $id_solicitud . ' 
            and a.id_usuario =' . session('id_usuario') . '
            ) and c.id_tramite=10 and c.id_documento=' . $id_documento);
    ...
    public static function consulta_archivos_faltantes($id_solicitud)
    {
        ...
        and a.id_solicitud =' . $id_solicitud . '         
        and a.id_usuario =' . session('id_usuario') . '
        ) and c.id_documento>0 and c.obligatorio=1 and c.id_Tramite=10');
    ...
    {
        $sql = "execute diu_consulta_requisitos_validados_vdigital ?";
        $result = DB::connection('captura_op')->select($sql, [$id_solicitud]);
        return $result;
    }

    public static function consulta_requisitos_ord($id_solicitud)
    {
    ...
    public static function notifica($request, $titulo, $mensaje, $correo)
    {
        ...
        if (DB::table('notificaciones')->insert($data)) {
            ...
            return DB::table('solicitudes')
                ...
                ->update(['id_etapa' => 58, 'estatus' => 'en revision']);
        } else {
            return false;
        }
    }
